...
/// `DeviceFlowAuthenticatorTests`専用の`URLProtocol`スタブ。
///
/// `GitHubClientTests`の`StubURLProtocol`と実装は同じだが、静的なスタブ格納領域を
/// クラスごとに分離するために別クラスとして用意している。両スイートは`swift test`で
/// 並列実行されうるため、単一クラスを共有すると片方の`reset()`がもう片方の登録済み
/// スタブを消してしまうレースコンディションが発生する。
final class DeviceFlowStubURLProtocol: URLProtocol {
