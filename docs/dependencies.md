...
| パッケージ | 検討理由 | 不採用の理由 |
|---|---|---|
| [SwiftyJSON/SwiftyJSON](https://github.com/SwiftyJSON/SwiftyJSON) | 検討候補として挙がった | 動的スキーマの緩いJSON処理に強みがあるが、CairnはGitHub APIの構造化レスポンスを`Codable`で型安全に扱う設計方針。緩い操作の利点を活かす場面がなく、Apple公式API優先の方針にも合わない |
| [SwifterSwift/SwifterSwift](https://github.com/SwifterSwift/SwifterSwift) | 検討候補として挙がった | 500以上の汎用Swift/Foundation拡張。全部をまとめて依存に加えるのは「極力Apple公式・最小依存」の方針に反する。必要な拡張（相対時間表示、文字列トリム等）はその都度自前実装する方針 |
| [SFSafeSymbols](https://github.com/SFSafeSymbols/SFSafeSymbols) | 実装計画の初期検討時に候補として挙がった | 最終的に不採用を確定。`Image(systemName:)`の直書きで対応する |
...
3. **依存はタグ付きリリースではなく特定コミットに`revision:`固定している**（`Package.swift`参照）。タグが無いため`branch: "main"`で追跡すると、mainブランチへの将来の任意のpushが無検証で取り込まれるsupply-chainリスクがあるため。

---

## バージョン管理・ビルド運用（先行事例からの学び）

Swift製macOSアプリを個人開発している他プロジェクトの構成を調査し、以下を踏襲する。

### `Package.swift`のみでの運用

`.xcodeproj`を持たず、`Package.swift`の`executableTarget`でGUIアプリ本体を直接定義する構成が、実際にシップされているmacOSアプリの先行事例として存在することを確認した。Cairnもこれに倣い`Cairn.xcodeproj`を作らず、`Package.swift`一本で依存関係とターゲットを管理する（実装計画セクション10.7）。
...
先行事例のうち、単一チャンネル運用のものはリポジトリ直下に`version.env`という単純なキーバリューファイルを置き、ビルドスクリプト・CIがそこからバージョン番号を読み取っている。もう一方の事例はStable/Betaの2チャンネルで運用しており、それぞれのチャンネル用のバージョン・ビルド番号をキーとして持つ形式だった。

Cairnは初期段階では**単一チャンネル方式**（`version.env`に`MARKETING_VERSION`/`BUILD_NUMBER`のみ）を採用する（`version.env`参照）。理由: 新規プロジェクトでBeta配信チャンネルを最初から二重運用するのはYAGNIに反する。将来Beta配信が必要になれば、`version.env`にキーを追加するだけで2チャンネル方式に移行できる。

### dmgパッケージング

先行事例の一つが使っていた`create-dmg`ツール（`brew install create-dmg`が前提）によるdmg生成スクリプトの構成を参考に、Cairnでも同様のスクリプトを`scripts/build-dmg.sh`として用意する（Phase 10で実装）。背景画像・アイコン配置・ドロップリンクを含むdmgを生成できる。

### 署名について（重要な差分）

先行事例の一つはDeveloper ID署名＋公証を行っていたが、**Cairnはこの部分を踏襲しない。** ユーザーはApple Developer Programに加入しておらずDeveloper ID証明書を持っていないため、Cairnは**完全無署名配布**とする（実装計画セクション10.5）。先行事例のビルドスクリプトからは、バージョン管理・dmg生成・appcast更新の仕組みだけを流用し、コード署名・公証のステップは含めない。

---

## オンボーディングUIの参考

権限リクエストを含むステップ形式のオンボーディング画面を持つ別プロジェクトの構成を参考にする:

- 権限行コンポーネント: `.granted` / `.needsAction` / `.blocked`という3状態のenumで権限行の見た目を切り替えるパターン。Required/Optionalバッジ、`.blocked`時のコーナーバッジ（テキストピルではなく警告アイコン+ツールチップ）というデザイン上の工夫がある
- ステップ形式オンボーディングの共通ラッパーとデザインシステムの分離（ステップコンテナ、背景コンポーネント、デザイントークンを別ファイルに分ける構成）
...
参考プロジェクトの3状態権限行パターンをそのままCairnの権限行UI（App管理権限の許可状態表示）に転用する。
